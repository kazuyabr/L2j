		...
		if (skills != null)
		{
			...
			for (int i = 0; i < skillsSplit.length; ++i)
			{
				try
				{
					...
					if (id == 0)
					{
						_log.info("Ignoring item_skill(" + skillsSplit[i] + ") for item " + toString() + ". Skill id is 0.");
						continue;
					}
					
					if (level == 0)
					{
						_log.info("Ignoring item_skill(" + skillsSplit[i] + ") for item " + toString() + ". Skill level is 0.");
						continue;
					}
				...
				{
					_log.warning("Failed to parse item_skill(" + skillsSplit[i] + ") for item " + toString() + ". The used format is wrong.");
				}
			}
